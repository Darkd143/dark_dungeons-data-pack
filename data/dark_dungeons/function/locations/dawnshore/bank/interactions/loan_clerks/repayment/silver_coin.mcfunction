...
scoreboard players remove @s loan_money_1 10

execute if entity @s[scores={loan_money_1=1..}] run tellraw @s [{"text":"Loan Repayment Clerk: You have payed off 10 Eni, "},{selector:"@s"},{"text":". You have "},{"score":{name:"@s","objective":"loan_money_1"}},{"text":" Eni in loans that still needs to be payed off."}]
