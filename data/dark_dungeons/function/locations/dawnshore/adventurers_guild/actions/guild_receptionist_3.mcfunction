...
# No quest assigned
execute if entity @s[scores={adventurer_rank=0..}] unless entity @s[scores={quest_id=1..}] run tellraw @s ["Canceling your quest isn't ideal, but it's preferable to failing outright. If you need assistance with canceling, I'm here to help." ]

# Cannot Cancel completed Quest
execute if entity @s[scores={adventurer_rank=0..,quest_id=1..,task_progress=100}] run tellraw @s ["It appears you have already completed your quest. I cannot cancel quests that are completed."]

# Canceling Quest
execute if entity @s[scores={adventurer_rank=0..,quest_id=1..}] unless score @s task_progress matches 100 run function dark_dungeons:locations/dawnshore/adventurers_guild/board/actions/cancel_quest