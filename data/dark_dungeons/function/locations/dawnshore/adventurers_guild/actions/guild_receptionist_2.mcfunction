...
execute if entity @s[scores={adventurer_rank=0..,temp_storage=0}] unless entity @s[scores={quest_id=1..}] unless score @s quest_day_limit matches 0 run tellraw @s ["Guild Receptionist Nessa: Hello, ",{"selector":"@s"},"! Please select a quest from the quest board, and I will assist in assigning it to you."]
...
# Assign Quest
execute if entity @s[scores={adventurer_rank=0..,temp_storage=1..8}] unless entity @s[scores={quest_id=1..}] unless score @s quest_day_limit matches 0 run function dark_dungeons:locations/dawnshore/adventurers_guild/board/actions/assign_quest
...
execute if entity @s[scores={adventurer_rank=0..}] unless entity @s[scores={quest_id=1..}] if score @s quest_day_limit matches 0 run tellraw @s ["Your adventurer status has been canceled due to inactivity. To accept quests again, please re-register as an adventurer. Elara can assist you with the re-registration process."]
