...
execute at @n[type=minecraft:interaction,tag=dawnshore_farmer_outside_interaction] if entity @n[distance=..10,tag=dawnshore_quest_farmer_text_display] unless entity @a[distance=..10,scores={quest_id=7}] run function dark_dungeons:locations/dawnshore/pier/quest_alert/teardown
