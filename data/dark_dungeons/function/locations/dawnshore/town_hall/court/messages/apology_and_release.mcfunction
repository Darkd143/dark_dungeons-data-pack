...
execute as @a[predicate=dark_dungeons:positioning/locations/dawnshore/town_hall/courtroom_cell] unless entity @s[scores={left_game=1..}] run tellraw @a[predicate=dark_dungeons:positioning/locations/dawnshore/town_hall/court_room] [{"text":"Judge Thalric: It seems apologies are in order "},{"selector":"@s"},{"text":". You will be released immediately."}]

execute as @a[predicate=dark_dungeons:positioning/locations/dawnshore/town_hall/courtroom_cell] unless entity @s[scores={left_game=1..}] run function dark_dungeons:players/crime/court/clear_player_charges

schedule function dark_dungeons:locations/dawnshore/town_hall/court/release_player_identifier 50
