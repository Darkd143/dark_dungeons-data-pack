# Innkeeper
execute as @e[predicate=dark_dungeons:positioning/locations/dawnshore/inn_zone,tag=dawnshore_inn_innkeeper_interaction] store success entity @s interaction.player[] int 0 on target if entity @s[tag=!dawnshore_inn_client] if entity @s[predicate=!dark_dungeons:inventory/holding_item/currency/copper_coin] run tellraw @s {"text":"Welcome to Dawnshore Inn! A cozy room costs just one copper coin per night, and you can store your belongings safely there. To rent a room, please hand over one copper coin. Let me know when you're ready to settle in!"}
execute as @e[predicate=dark_dungeons:positioning/locations/dawnshore/inn_zone,tag=dawnshore_inn_innkeeper_interaction] store success entity @s interaction.player[] int 0 on target if entity @s[tag=!dawnshore_inn_client] if entity @s[predicate=dark_dungeons:inventory/holding_item/currency/copper_coin] run tag @s add dawnshore_inn_new_client_action
