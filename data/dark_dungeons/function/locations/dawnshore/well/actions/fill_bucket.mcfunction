...
setblock -137 73 86 iron_chain[axis=y,waterlogged=false]

# Quest Progress Update
execute unless entity @s[scores={quest_id=4..5}] run return 1

execute if entity @s[scores={quest_id=4,task_progress=20}] run scoreboard players set @s task_progress 40
