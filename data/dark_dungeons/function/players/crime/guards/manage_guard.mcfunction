...
execute at @s unless entity @a[distance=0..10,gamemode=!spectator,gamemode=!creative,predicate=dark_dungeons:player_state/criminal/is_targetable_criminal] run return run function dark_dungeons:players/crime/guards/remove_and_replace

execute at @s if entity @s[predicate=dark_dungeons:targetting/entity_targetting_player] on target unless entity @s[distance=0..10,gamemode=!spectator,gamemode=!creative,predicate=dark_dungeons:player_state/criminal/is_targetable_criminal] as @n[tag=guard] run function dark_dungeons:players/crime/guards/select_new_target
